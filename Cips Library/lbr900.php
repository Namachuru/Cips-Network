...
<BODY>
	...
	<div class="wrapper">
		<img class="img-1stpic" src = "Graphics\lbr900.png" alt"Family"></img>
		<article class="img-info">
		<h2>900 - History & Geography</h2>
		<br/>
		<table>
		<tr>
		...
		$query = 'SELECT * FROM ct900 ORDER by id ASC';	
		...
		$que = 'SELECT * FROM ct900 ORDER by id ASC LIMIT ' . $this_page_first_result . ',' . $results_per_page;
		$fesult = mysqli_query($connect, $que);
		
		if ($fesult):
			if(mysqli_num_rows($fesult)>0):
				while($ct900 = mysqli_fetch_assoc($fesult)):
				?>
					<tr>
					<td><a href = "<?php echo $ct900['web link']; ?>"><?php echo $ct900['book title']; ?></a></td><td><?php echo $ct900['author']; ?></td><td><?php echo $ct900['pub year']; ?></td><td><?php echo $ct900['pub loc']; ?></td><td>
					<?php
					$sellable = $ct900['sellable'];
					if ($sellable):
					?>
						<a href = "Shop\buy_<?php echo $ct900['sell link']; ?>">Buy Now!</a></td>
					<?php
					endif;
					if (!$sellable):
					?>
		...
		for ($page=1;$page<=$number_of_pages; $page++){
			echo '<a href="lbr900.php?page=' . $page . '">' . $page . '</a> ';
		}
		?>
		</article>
