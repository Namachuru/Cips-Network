...
<BODY>
	...
	<div class="wrapper">
		<img class="img-1stpic" src = "Graphics\lbr400.png" alt"Family"></img>
		...
		$query = 'SELECT * FROM ct400 ORDER by id ASC';	
		...
		$que = 'SELECT * FROM ct400 ORDER by id ASC LIMIT ' . $this_page_first_result . ',' . $results_per_page;
		$fesult = mysqli_query($connect, $que);
		
		if ($fesult):
			if(mysqli_num_rows($fesult)>0):
				while($ct400 = mysqli_fetch_assoc($fesult)):
				?>
					<tr>
					<td><a href = "Lib\<?php echo $ct400['web link']; ?>"><?php echo $ct400['book title']; ?></a></td><td><?php echo $ct400['author']; ?></td><td><?php echo $ct400['pub year']; ?></td><td><?php echo $ct400['pub loc']; ?></td><td>
					<?php
					$sellable = $ct400['sellable'];
					if ($sellable):
					?>
						<a href = "Shop\buy_<?php echo $ct400['web link']; ?>">Buy Now!</a></td>
					<?php
					endif;
					if (!$sellable):
					?>
		...
		for ($page=1;$page<=$number_of_pages; $page++){
			echo '<a href="lbr400.php?page=' . $page . '">' . $page . '</a> ';
		}
		?>
		</article>
	</div>	
