...
<BODY>
	...
	<div class="wrapper">
		<img class="img-1stpic" src = "Graphics\lbr800.png" alt"Family"></img>
		<article class="img-info">
		<h2>800 - Literature</h2>
		<br/>
		<table>
		<tr>
		...
		$query = 'SELECT * FROM ct800 ORDER by id ASC';	
		...
		$que = 'SELECT * FROM ct800 ORDER by id ASC LIMIT ' . $this_page_first_result . ',' . $results_per_page;
		$fesult = mysqli_query($connect, $que);
		
		if ($fesult):
			if(mysqli_num_rows($fesult)>0):
				while($ct800 = mysqli_fetch_assoc($fesult)):
				?>
					<tr>
					<td><a href = "Lib\<?php echo $ct800['web link']; ?>"><?php echo $ct800['book title']; ?></a></td><td><?php echo $ct800['author']; ?></td><td><?php echo $ct800['pub year']; ?></td><td><?php echo $ct800['pub loc']; ?></td><td>
					<?php
					$sellable = $ct800['sellable'];
					if ($sellable):
					?>
						<a href = "Shop\buy_<?php echo $ct800['web link']; ?>">Buy Now!</a></td>
					<?php
					endif;
					if (!$sellable):
					?>
		...
		for ($page=1;$page<=$number_of_pages; $page++){
			echo '<a href="lbr800.php?page=' . $page . '">' . $page . '</a> ';
		}
		?>
		</article>
	</div>	
