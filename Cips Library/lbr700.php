...
<BODY>
	...
	<div class="wrapper">
		<img class="img-1stpic" src = "Graphics\lbr700.png" alt"Family"></img>
		<article class="img-info">
		<h2>700 - Arts & Recreation</h2>
		<br/>
		<table>
		<tr>
		...
		$query = 'SELECT * FROM ct700 ORDER by id ASC';	
		...
		$que = 'SELECT * FROM ct700 ORDER by id ASC LIMIT ' . $this_page_first_result . ',' . $results_per_page;
		$fesult = mysqli_query($connect, $que);
		
		if ($fesult):
			if(mysqli_num_rows($fesult)>0):
				while($ct700 = mysqli_fetch_assoc($fesult)):
				?>
					<tr>
					<td><a href = "Lib\<?php echo $ct700['web link']; ?>"><?php echo $ct700['book title']; ?></a></td><td><?php echo $ct700['author']; ?></td><td><?php echo $ct700['pub year']; ?></td><td><?php echo $ct700['pub loc']; ?></td><td>
					<?php
					$sellable = $ct700['sellable'];
					if ($sellable):
					?>
						<a href = "Shop\buy_<?php echo $ct700['web link']; ?>">Buy Now!</a></td>
					<?php
					endif;
					if (!$sellable):
					?>
		...
		for ($page=1;$page<=$number_of_pages; $page++){
			echo '<a href="lbr700.php?page=' . $page . '">' . $page . '</a> ';
		}
		?>
		</article>
	</div>	
