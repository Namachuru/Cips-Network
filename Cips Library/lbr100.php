...
<BODY>
	...
	<div class="wrapper">
		<img class="img-1stpic" src = "Graphics\lbr100.png" alt"Family"></img>
		<article class="img-info">
		<h2>100 - Philosophy & Psycology</h2>
		<br/>
		<table>
		<tr>
		...
		$query = 'SELECT * FROM ct100 ORDER by id ASC';	
		...
		$que = 'SELECT * FROM ct100 ORDER by id ASC LIMIT ' . $this_page_first_result . ',' . $results_per_page;
		$fesult = mysqli_query($connect, $que);
		
		if ($fesult):
			if(mysqli_num_rows($fesult)>0):
				while($ct100 = mysqli_fetch_assoc($fesult)):
				?>
					<tr>
					<td><a href = "Lib\<?php echo $ct100['web link']; ?>"><?php echo $ct100['book title']; ?></a></td><td><?php echo $ct100['author']; ?></td><td><?php echo $ct100['pub year']; ?></td><td><?php echo $ct100['pub loc']; ?></td><td>
					<?php
					$sellable = $ct100['sellable'];
					if ($sellable):
					?>
						<a href = "Shop\buy_<?php echo $ct100['web link']; ?>">Buy Now!</a></td>
					<?php
					endif;
					if (!$sellable):
					?>
		...
		for ($page=1;$page<=$number_of_pages; $page++){
			echo '<a href="lbr100.php?page=' . $page . '">' . $page . '</a> ';
		}
		?>
		</article>
	</div>	
