...
<BODY>
	...
	<div class="wrapper">
		<img class="img-1stpic" src = "Graphics\lbr300.png" alt"Family"></img>
		<article class="img-info">
		<h2>300 - Social Sciences</h2>
		<br/>
		<table>
		<tr>
		<td>Book</td><td>Author</td><td>Publish Date</td><td>Publication Location</td><td>Purchase</td>
		</tr>
		<?php
		...
		$query = 'SELECT * FROM ct300 ORDER by id ASC';	
		...
		$que = 'SELECT * FROM ct300 ORDER by id ASC LIMIT ' . $this_page_first_result . ',' . $results_per_page;
		$fesult = mysqli_query($connect, $que);
		
		if ($fesult):
			if(mysqli_num_rows($fesult)>0):
				while($ct300 = mysqli_fetch_assoc($fesult)):
				?>
					<tr>
					<td><a href = "Lib\<?php echo $ct300['web link']; ?>"><?php echo $ct300['book title']; ?></a></td><td><?php echo $ct300['author']; ?></td><td><?php echo $ct300['pub year']; ?></td><td><?php echo $ct300['pub loc']; ?></td><td>
					<?php
					$sellable = $ct300['sellable'];
					if ($sellable):
					?>
						<a href = "Shop\buy_<?php echo $ct300['web link']; ?>">Buy Now!</a></td>
					<?php
					endif;
					if (!$sellable):
					?>
		...
		for ($page=1;$page<=$number_of_pages; $page++){
			echo '<a href="lbr300.php?page=' . $page . '">' . $page . '</a> ';
		}
		?>
		</article>
	</div>	
